...
public class HookHelper {
    ...
    /**
     * 方法一:
     * 通过替换Instrumentation实现(此方法比较简单,但不好,由于Instrumentation是Activity的成员变量,而Activity在程序中不止一个实例,所以需要在每个actviity的实例中都得执行一遍)
     * @param activity
     */
    public static void doInstrumentationHook(Activity activity){
        try {
            ...
            mInstrumentationField.set(activity, new EvilInstrumentation(mInstrumentation));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
